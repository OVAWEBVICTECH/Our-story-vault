...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose, onRefreshVault }) => {
  ...
  const [isGeneratingLoveLetter, setIsGeneratingLoveLetter] = useState(false);
  ...
  const handleGenerateLoveLetter = async () => {
    setIsGeneratingLoveLetter(true);
    try {
      const res = await fetch('/api/admin/ai/loveletter', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          occasionTitle,
      ...
      if (data.success && data.letter) {
        setSettings((prev) => ({
          ...prev,
          loveLetterTitle: data.letter.title,
          loveLetterBody: data.letter.body,
        }));
        showToast('✨ Wax seal letter auto-generated by AI matching occasion!');
      }
    } catch (err) {
      showToast('Failed to generate love letter');
    } finally {
      setIsGeneratingLoveLetter(false);
    }
  };
                  ...
                  <div className="space-y-4 bg-white/5 p-6 rounded-2xl border border-rose-500/20">
                    ...
                    <div className="space-y-2 pt-2">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        <label className="block text-xs font-medium text-rose-300">Wax Seal Love Letter Message Body</label>
                        <button
                          type="button"
                          onClick={handleGenerateLoveLetter}
                          disabled={isGeneratingLoveLetter}
                          className="px-3 py-1 rounded-xl bg-gradient-to-r from-rose-500/30 via-pink-500/30 to-rose-500/30 hover:from-rose-500/50 hover:to-pink-500/50 border border-rose-400/40 text-rose-200 text-xs font-semibold flex items-center gap-1.5 cursor-pointer disabled:opacity-50 transition-all active:scale-95 shadow-sm"
                          title="Auto-generate message matching occasion with sweet words, prayers & pickup lines using Gemini AI"
                        >
                          <Sparkles className={`w-3.5 h-3.5 text-amber-300 ${isGeneratingLoveLetter ? 'animate-spin' : 'animate-pulse'}`} />
                          <span>{isGeneratingLoveLetter ? 'Generating Letter...' : 'AI Auto-Generate for Occasion'}</span>
                        </button>
                      </div>
                      ...
                      <textarea
                        ...
                        placeholder="Write or auto-generate your wax seal love letter message..."
                        className="w-full px-3 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white text-xs font-serif leading-relaxed resize-none focus:outline-none focus:border-rose-400"
                      />
                    </div>
